...
export function GuestPortal() {
  ...
  if (access && restaurant) {
    ...
    return <main style={restaurantThemeStyle(restaurant)} className="dark min-h-screen bg-background text-foreground">
      <header className="border-b border-white/10 bg-[var(--brand-surface)] text-white"><div className="mx-auto flex max-w-4xl items-center justify-between gap-4 px-5 py-4"><Link href={`/it/book/${restaurant.slug}`} className="block shrink-0"><BrandLogo restaurant={restaurant} priority subtitle="Area ospite" /></Link><Button variant="outline" size="sm" onClick={() => setAccess(null)}>Esci</Button></div></header>
  ...
  return <main className="dark min-h-screen bg-[#0C0C0B] text-white"><section className="japanese-pattern mx-auto grid min-h-screen max-w-6xl items-center gap-10 px-5 py-12 lg:grid-cols-[minmax(0,1fr)_440px]"><div className="max-w-xl"><div><BrandLogo priority subtitle="Area ospite" /></div><p className="mt-12 font-mono text-xs uppercase tracking-[0.26em] text-primary">Area ospite</p><h1 className="mt-4 font-heading text-5xl leading-[1.02] tracking-tight sm:text-6xl">La tua prenotazione, sempre a portata di mano.</h1><p className="mt-6 text-base leading-7 text-white/60">Accedi con il codice ricevuto e il numero di telefono usato in prenotazione. Vedrai subito ristorante, orario e stato della richiesta.</p><div className="mt-8 grid gap-3 text-sm text-white/65 sm:grid-cols-2"><span className="flex items-center gap-2"><ShieldCheck className="size-4 text-primary" />Verifica protetta</span><span className="flex items-center gap-2"><CircleUserRound className="size-4 text-primary" />YUKO e KouSushi</span></div></div>
