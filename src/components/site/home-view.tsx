...
export function HomeView({ restaurant, site }: { restaurant: RestaurantLocation; site: SiteData }) {
  ...
  return <>
    ...
    <section className="relative overflow-hidden">
      ...
      <div className="mx-auto grid max-w-6xl items-center gap-12 px-5 pb-16 pt-16 lg:grid-cols-[1.05fr_.95fr] lg:pb-24 lg:pt-24">
        <div>
          <p className="mb-5 inline-flex items-center gap-2 rounded-full border border-primary/25 bg-primary/10 px-3.5 py-1.5 text-xs font-medium text-primary">
            <Sparkles className="size-3.5" />{restaurant.shortName} · {restaurant.serviceNote.split("·")[0].trim()} · {restaurant.city.split("·")[0].trim()}
          </p>
          <h1 className="max-w-2xl text-balance font-heading text-[2.6rem] font-semibold leading-[1.03] tracking-[-0.04em] sm:text-6xl lg:text-7xl">
            Il Giappone, servito con cura.
          </h1>
          <p className="mt-6 max-w-xl text-lg leading-7 text-white/60">
            Cucina giapponese e fusion in un ambiente curato ad {restaurant.city.split("·")[0].trim()}, con ampio parcheggio privato e prenotazione online immediata.
          </p>
          <div className="mt-9 flex flex-wrap gap-3">
            ...
            <Link href="/dove-siamo" className="inline-flex min-h-13 items-center gap-2 rounded-full border border-white/15 px-6 text-base font-medium text-white transition-colors hover:border-primary/50 hover:bg-white/[0.03]">
              <MapPin className="size-4 text-primary" />Come arrivare
            </Link>
          </div>
        </div>
        <PhotoPanel restaurant={restaurant} label="La sala" className="min-h-[340px] lg:min-h-[440px]" />
      </div>
    </section>
    ...
    <section className="border-y border-white/8 bg-white/[0.015]">
      <div className="mx-auto grid max-w-6xl gap-px overflow-hidden bg-white/8 sm:grid-cols-3">
        {highlights.map((item) => <div key={item.title} className="bg-background px-6 py-8">
    ...
    <section className="mx-auto grid max-w-6xl items-center gap-12 px-5 py-20 lg:grid-cols-2 lg:py-28">
      <PhotoPanel restaurant={restaurant} label="L’atmosfera" className="order-2 min-h-[300px] lg:order-1 lg:min-h-[420px]" />
    ...
    <section className="border-t border-white/8 bg-white/[0.015]">
      <div className="mx-auto max-w-6xl px-5 py-20 lg:py-24">
        ...
        <div className="mt-12 grid gap-5 sm:grid-cols-3">
          ...
          ].map((step) => <div key={step.n} className="surface-3d-dark rounded-2xl border border-white/10 bg-card p-6">
    ...
    <section className="mx-auto max-w-6xl px-5 py-20">
      <div className="grid gap-6 rounded-3xl border border-white/10 bg-card p-8 sm:p-10 lg:grid-cols-[1.3fr_1fr] lg:items-center">
        <div>
          <h2 className="font-heading text-3xl font-semibold tracking-tight">Ti aspettiamo ad {restaurant.city.split("·")[0].trim()}.</h2>
