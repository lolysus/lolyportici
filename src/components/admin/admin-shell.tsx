  ...
  return <div style={isMasterAccount ? undefined : restaurantThemeStyle(activeLocation)} className="dark min-h-screen bg-background text-foreground">
    ...
    <div className={cn("min-h-screen transition-[padding]", collapsed ? "md:pl-[76px]" : "md:pl-60")}>
      <header className="sticky top-0 z-30 flex h-[4.25rem] items-center gap-1.5 border-b border-white/8 bg-background/88 px-2.5 backdrop-blur-xl sm:h-16 sm:gap-3 sm:px-4 md:px-7">
        ...
        <button onClick={() => setCommandOpen(true)} className="flex size-11 shrink-0 touch-manipulation items-center justify-center rounded-lg border border-white/8 bg-card/60 text-muted-foreground hover:border-white/15 sm:min-w-0 sm:max-w-sm sm:flex-1 sm:justify-start sm:gap-2 sm:px-3 sm:text-sm lg:h-9" aria-label="Vai a una sezione"><Search className="size-4" /><span className="hidden truncate sm:inline">Vai a una sezione…</span><kbd className="ml-auto hidden rounded border border-white/10 px-1.5 font-mono text-[10px] md:block">Ctrl K</kbd></button>
        {isMasterAccount ? <Select value={activeLocation.slug} onValueChange={switchLocation} disabled={locationPending}>
          {/* L'altezza va imposta sulla stessa variante del componente:
              data-[size=default]:h-8 batte per specificità una semplice h-11,
              e il selettore restava a 32px invece dei 44 che serve un dito. */}
          <SelectTrigger aria-label="Cambia ristorante" className="w-11 justify-center border-white/8 bg-card/60 p-0 data-[size=default]:h-11 [&>span]:hidden sm:w-[176px] sm:justify-start sm:px-3 sm:data-[size=default]:h-11 sm:[&>span]:inline">
