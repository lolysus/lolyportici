...
export function LocationCards({ summaries, activeLocationId }: { summaries: LocationSummary[]; activeLocationId: string }) {
  ...
  function manageLocation(slug: string) {
    setPendingSlug(slug);
    setError(null);
    startTransition(async () => {
      ...
      if (!response.ok) {
        setPendingSlug(null);
        setError("Non è stato possibile aprire il ristorante. Riprova.");
        return;
      }
  ...
  return <>
    <div className="grid gap-5 xl:grid-cols-2">
      {summaries.map((summary, index) => {
        ...
        const pending = pendingSlug === location.slug;
        ...
        return <article key={location.id} style={restaurantThemeStyle(location)} className={cn("surface-3d-dark group relative overflow-hidden rounded-xl border border-t-2 border-t-primary bg-card", active ? "border-primary/55" : "border-white/8")}>
          ...
          <div className="relative flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between sm:px-7">
            ...
            <div className="flex gap-2">
              ...
              <Button size="sm" onClick={() => manageLocation(location.slug)} disabled={Boolean(pendingSlug)}>{pending ? <LoaderCircle className="animate-spin" /> : null}{active ? "Apri ristorante" : "Gestisci ristorante"}<ArrowRight /></Button>
            </div>
          </div>
        </article>;
      })}
    </div>
    {error && <p role="alert" className="mt-5 rounded-xl border border-destructive/20 bg-destructive/10 p-3 text-sm text-destructive">{error}</p>}
  </>;
}
