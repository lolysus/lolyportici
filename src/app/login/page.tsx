...
export default function LoginPage() {
  ...
  return <main className="grid min-h-screen bg-[#111] lg:grid-cols-[minmax(0,1.15fr)_minmax(430px,.85fr)]">
    <div className="relative hidden overflow-hidden border-r border-white/10 lg:flex lg:flex-col lg:justify-between lg:p-12">
      ...
      <div className="relative flex items-start justify-between"><div className="w-56 text-white"><BrandLogo priority /></div><span className="inline-flex items-center gap-2 rounded-full border border-emerald-400/20 bg-emerald-400/8 px-3 py-1.5 text-[10px] font-medium text-emerald-200"><span className="signal-pulse size-1.5 rounded-full bg-emerald-400" />Sistema operativo</span></div>
    ...
    <div className="relative flex items-center overflow-hidden bg-background px-5 py-12 sm:px-12"><div aria-hidden className="absolute -right-28 top-16 size-72 rounded-full bg-[radial-gradient(circle,rgba(228,98,77,.12),transparent_66%)]" /><div className="relative mx-auto w-full max-w-md"><div className="mb-10 w-52 lg:hidden"><BrandLogo priority /></div><LoginForm demoMode={demoMode} /></div></div>
  </main>;
}
