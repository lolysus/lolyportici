...
const workflow = [
  ...
  { icon: CalendarCheck2, title: "2. Controlla", text: "Apri l’agenda, verifica dati, allergie, note, orario e tavolo assegnato." },
  { icon: UsersRound, title: "3. Accogli", text: "Fai avanzare lo stato da confermata ad arrivata, al tavolo e infine completata." },
];
...
export default async function HelpPage() {
  ...
  return <>
    ...
    <section className="surface-3d-dark overflow-hidden rounded-2xl border bg-card">
      <div className="border-b p-5 sm:p-6"><p className="font-mono text-[10px] uppercase tracking-[0.2em] text-primary">Flusso quotidiano</p><h2 className="mt-2 font-heading text-2xl">Dalla richiesta al tavolo</h2></div>
      <div className="grid gap-px bg-border md:grid-cols-3">{workflow.map((item) => <article key={item.title} className="bg-card p-5 sm:p-6"><span className="flex size-10 items-center justify-center rounded-xl bg-primary/10 text-primary"><item.icon className="size-4" /></span><h3 className="mt-5 font-semibold">{item.title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{item.text}</p></article>)}</div>
    </section>
    <div className="mt-6 grid gap-6 lg:grid-cols-[1.2fr_.8fr]">
      <section className="rounded-2xl border bg-card p-5 sm:p-6"><h2 className="font-heading text-2xl">Checklist di apertura</h2><ul className="mt-5 space-y-3">{[
        "Verifica che l’indicatore in alto sia Live o che il controllo periodico sia attivo.",
        "Apri il centro notifiche e abilita il suono sul dispositivo dedicato.",
        "Controlla agenda, coperti previsti, allergie e gruppi numerosi.",
        "Conferma che la sede selezionata in alto sia quella corretta.",
        "Se necessario, sospendi il booking da Impostazioni senza perdere prenotazioni o configurazione.",
      ].map((item) => <li key={item} className="flex gap-3 text-sm leading-6"><CheckCircle2 className="mt-1 size-4 shrink-0 text-emerald-400" /><span>{item}</span></li>)}</ul></section>
