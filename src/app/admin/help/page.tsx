...
export default async function HelpPage() {
  ...
  return <>
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[1.2fr_.8fr]">
      <section className="rounded-2xl border bg-card p-5 sm:p-6"><h2 className="font-heading text-2xl">Checklist di apertura</h2><ul className="mt-5 space-y-3">{["Verifica che l’indicatore in alto sia Live o che il controllo periodico sia attivo.", "Apri il centro notifiche e abilita il suono sul dispositivo dedicato.", "Controlla agenda, coperti previsti, allergie e gruppi numerosi.", "Se necessario, sospendi il booking da Impostazioni senza perdere prenotazioni o configurazione."].map((item) => <li key={item} className="flex gap-3 text-sm leading-6"><CheckCircle2 className="mt-1 size-4 shrink-0 text-emerald-400" /><span>{item}</span></li>)}</ul></section>
      <section className="rounded-2xl border bg-card p-5 sm:p-6"><h2 className="font-heading text-2xl">Azioni rapide</h2><div className="mt-5 grid gap-3"><Button asChild className="justify-between"><Link href="/admin/reservations"><span className="flex items-center gap-2"><CalendarCheck2 />Apri agenda</span><ExternalLink /></Link></Button>{session.permissions.includes("settings:write") && <Button asChild variant="outline" className="justify-between"><Link href="/admin/settings"><span className="flex items-center gap-2"><Settings2 />Regole di sede</span><ExternalLink /></Link></Button>}<Button asChild variant="outline" className="justify-between"><Link href={`/prenota/${location.slug}`}><span className="flex items-center gap-2"><MapPin />Vedi la pagina prenotazioni</span><ExternalLink /></Link></Button></div><p className="mt-5 flex gap-2 rounded-xl border border-primary/15 bg-primary/5 p-3 text-xs leading-5 text-muted-foreground"><ShieldCheck className="mt-0.5 size-4 shrink-0 text-primary" />Questo pannello è solo di {location.shortName}: prenotazioni, sala e ospiti degli altri locali non sono raggiungibili da qui.</p></section>
    </div>
  </>;
}
