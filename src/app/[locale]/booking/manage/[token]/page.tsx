...
export default async function ManagePage({ params }: { params: Promise<{ locale: string; token: string }> }) {
  ...
  return <div style={restaurantThemeStyle(location)} className="dark min-h-screen bg-background"><header className="bg-[var(--brand-surface)] px-5 py-4 text-white"><div className="mx-auto w-52"><BrandLogo priority restaurant={location} subtitle="Area ospite" /></div></header><ManageBooking token={token} locale={locale} /></div>;
}
