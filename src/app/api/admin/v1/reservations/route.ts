...
export async function GET(request?: Request) {
  try { const session = await requirePermission("reservations:read"); const location=getAdminLocationFromRequest(request, session); return success(await getRepository(location.id).listReservations()); }
